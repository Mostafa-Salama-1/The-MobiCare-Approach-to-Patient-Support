...
# ---------------- UART CONFIGURATION ----------------
SERIAL_PORT = 'COM3'  # Change this to your STM serial port
BAUD_RATE = 9600
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    down_threshold = 25
                    # print(f"DEBUG: y_nose={y_nose}, eye_avg_y={eye_avg_y}, y_nose-eye_avg_y={y_nose-eye_avg_y}")
                    if y_nose < eye_avg_y:
                        direction = "up"
...
def uart_mode():
    ...
    try:
        ...
        return
    print("UART mode started. Send '3' for voice, '4' for face. The last command remains active until interrupted by another.")
    active_mode = None
    try:
        while True:
            ...
            if ser.in_waiting:
                ...
                if cmd == '3':
                    ...
                    ser.write(b'3')  # Send confirmation to STM
                elif cmd == '4':
                    active_mode = 'face'
                    print("Switched to face mode.")
                    ser.write(b'4')  # Send confirmation to STM
                elif cmd.lower() == 'q':
                    print("Exiting UART mode.")
                    break
                else:
            ...
            if active_mode == 'voice':
                ...
                ser.write((result + '\n').encode('utf-8'))
                print(f"Sending direction serially: {result}")
            elif active_mode == 'face':
                print("Running face detection...")
                result = run_face_detection_stream(send_callback=lambda direction: ser.write((direction + '\n').encode('utf-8')), interrupt_callback=lambda: False)
                print(f"Face result: {result}")
                print(f"Sending direction serially: {result}")
            time.sleep(0.1)
